...
OSM_PATH = "Clean_SilverSpringMap.xml"
SAMPLE_PATH = "SilverSpring_sample.xml"
...
# way tag keys are not audited
# {'lower': 280, 'lower colon': 181, 'others': 53, 'problemchars': 0, 'total': 514}
# others contain number and multiple colons

SCHEMA_PATH = "myschema.py"
...
# data is cleaned in separate py files
...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node': #audit_node_tags
        for key in element.attrib: 
            if key in NODE_FIELDS:
                node_attribs[key] = element.attrib[key] #only use selected 
        if element.find("tag") != -1:
            for tag in element.iter("tag"): 
                node_tag = {}  
                if tag.attrib['k'].find(':'): #contain 1 or more colons, not consider name with numbers 
        ...
        if element.find("tag") != -1:
            for tag in element.iter("tag"): 
                ...
                if problemchars.search(tag.attrib['k']) == None: # ignore problem keys in way tags
                    way_tag = {}
